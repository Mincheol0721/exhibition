...
<html>
	<head>
		...
		<script type="text/javascript">
			$(function() {
				$.ajax({
					url: '<%=request.getContextPath()%>/appForm/getList.do',
					data: {pageNum:<%=pageNum%>, pageSize:<%=pageSize%>, cname:'마이크로소프트'},
					async: false,
					type: 'POST',
					dataType: 'json',
					success: function(data) {
						$.each(data, function(index, vo) {
							$('.layout').append('<div class="myCard" onclick="location.href=\'${path}/view/viewAppForm.jsp?no=' + vo.no + '\'">' 
													+ '<div class="innerCard">'
														+ '<div class="frontSide"> '
														...
														+ '<div class="backSide"> '
															+ '<p><ul>'
																...
																+ '<li>장소: ' + vo.locate + '</li>'
																+ '<button type="button" style="background-color: #caf0f8; color: #0077b6;" onclick="location.href=\'${path}/view/modappForm.jsp?no=' + vo.no + '\'">'
																+ '수정하기'
																+ '</button> &nbsp;&nbsp;&nbsp;'
																+ '<button type="button" style="background-color: #caf0f8; color: #0077b6;" onclick="location.href=\'${path}/appForm/delappForm.do?no=' + vo.no + '\'">'
																+ '삭제하기'
																+ '</button>'
															+ '</ul></p>'
														+ '</div>'
